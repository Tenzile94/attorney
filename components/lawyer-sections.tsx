...
const CtaButton = ({
  ...
  <>
    <Link
      href="#contact"
      className={`hidden md:inline-flex items-center gap-2 ${className}`}
    >
      {children}
    </Link>
    <a
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 pt-16 pb-14 md:py-20 grid md:grid-cols-2 gap-16 items-center w-full">
        <div className="text-center md:text-left">
          ...
          <FadeUp delay={0.4}>
            <div className="flex flex-col sm:flex-row gap-3 justify-center md:justify-start">
              ...
              <Link
                href="/about"
                className="px-7 py-3.5 border border-white/30 text-white font-medium hover:bg-white/10 transition rounded text-sm"
              >
                {t.hero.about}
              </Link>
            </div>
        ...
        >
          <Image
            ...
            alt="Lawyer"
            fill
            className="object-cover rounded-xl shadow-[0_24px_80px_rgba(0,0,0,0.6)]"
          />
        </SlideIn>
      </div>
    </section>
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          ...
          <SlideIn from="right" delay={0.1}>
            <div>
              <p className="text-base text-accent mb-3 uppercase tracking-widest">
                {t.about.label}
              </p>
              ...
              <Link
                href="/about"
                className="inline-block px-8 py-3 bg-accent text-black font-semibold hover:opacity-90 transition rounded"
              >
                {t.about.btn}
              </Link>
            </div>
    ...
    <section className="py-32 relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4">
        ...
        <StaggerList className="grid sm:grid-cols-2 lg:grid-cols-4 gap-x-12 gap-y-16">
          {(t.why.reasons as readonly { title: string; desc: string }[]).map(
            (item, i) => (
              <StaggerItem key={i}>
        ...
        <FadeUp
          delay={0.3}
          className="mt-24 flex flex-col md:flex-row items-center justify-between gap-6 border-t border-white/10 pt-12"
        >
          <div>
            <p className="font-serif font-bold text-2xl text-white mb-1">
              {t.why.ctaTitle}
            </p>
...
export const PracticeSection = () => {
  ...
  return (
    <section className="py-32 bg-background">
      <div className="max-w-7xl mx-auto px-4">
        <FadeUp>
          <h2 className="text-4xl font-serif font-bold text-center mb-20 text-white">
            {t.practice.title}
          </h2>
        </FadeUp>
        ...
        <div className="mb-16">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {(
              ...
              <div
                key={i}
                className="p-8 bg-card border border-white/[0.07] card-glow rounded-xl flex items-start gap-4"
              >
                <div className="text-3xl">{item.icon}</div>
                <div>
                  <h4 className="font-semibold text-xl mb-1 text-white">
                    {item.title}
                  </h4>
        ...
        <div>
          <h3 className="text-2xl font-serif font-bold mb-8 text-accent">
            {t.practice.workTitle}
          </h3>
          <div className="grid md:grid-cols-3 gap-6">
            {(
              t.practice.work as readonly {
                year: string;
                place: string;
                role: string;
              }[]
              ...
              <div
                key={i}
                className="p-6 bg-card border border-accent/20 card-glow rounded-xl"
              >
                <div className="text-accent font-semibold text-base mb-2">
                  {item.year}
                </div>
    ...
    <section className="py-32 bg-card" id={id}>
      <div className="max-w-3xl mx-auto px-4">
        ...
        <div className="space-y-3">
          {(t.faq.items as readonly { q: string; a: string }[]).map(
            (faq, i) => (
              <div
                key={i}
                className="bg-background border border-white/[0.07] card-glow rounded-xl overflow-hidden"
              >
                <button
                  onClick={() => setExpanded(expanded === i ? null : i)}
                  className="w-full p-6 flex items-center justify-between hover:bg-white/[0.03] transition"
                >
                  <h3 className="font-semibold text-white text-left text-base">
                    {faq.q}
                  </h3>
                  <div className="text-accent flex-shrink-0 ml-4">
            ...
            <div className="space-y-8">
              ...
              {[
                  ...
                  lines: [
                    ...
                    >
                      <span className="text-white/50 text-xs">
                        {(t.contact as { officePhone: string }).officePhone}:
                      </span>{" "}
                      +994 12 498 95 95
                    </a>,
                    <a
                ...
                },
                {
                  ...
                  lines: ["abbas.karimbayli@gmail.com"],
                },
                {
                  icon: <MapPin className="w-6 h-6 text-accent" />,
                  label: t.contact.address,
                  lines: t.contact.addrLines as readonly string[],
                },
                {
                  icon: <Clock className="w-6 h-6 text-accent" />,
                  label: t.contact.hours,
                  lines: t.contact.hoursLines as readonly string[],
                },
              ].map((item) => (
                <div key={item.label} className="flex gap-4">
                  ...
                  <div>
                    <p className="font-semibold text-accent text-base mb-1">
                      {item.label}
                    </p>
                    {(item.lines as (string | React.ReactNode)[]).map(
                      (l, idx) => (
                        <div key={idx} className="text-white/70 text-base">
                          {l}
                        </div>
                      ),
                    )}
  ...
  return (
    <section className="bg-background">
      <div className="max-w-7xl mx-auto px-4 pb-16">
        ...
        <div
          className="rounded-2xl overflow-hidden border border-white/[0.07] shadow-[0_20px_60px_rgba(0,0,0,0.5)]"
          style={{ height: "480px" }}
        >
          {activeTab === "map" ? (
            <iframe
            ...
            <iframe
              key="tour"
              src="https://azerbaijan360.az/what-to-see/vekil-burolari/fides-v-kil-burosu-1/"
              style={{
                border: 0,
                width: "100%",
                height: "1100px",
